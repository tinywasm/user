...
3. `github.com/tinywasm/user/user/ui` (`userui`): UI modules and signal-bound logic.
...
- **Isomorphic modules:** Auth UI modules implement `site.Module` via duck typing. Backend modules (in `userserver`) handle SSR and `Create/Update/Delete`. Frontend modules (in `userui`) manage state via signal bindings.
...
- `user/ui/`: `UIModules()` for frontend registration and signal-bound reactive UI.
