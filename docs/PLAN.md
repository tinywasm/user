...
| 4 | [PLAN_AUTHORITY_RENAME.md](PLAN_AUTHORITY_RENAME.md) | ⛔ **BLOQUEADO** — hasta completar 1, 2 y 3 (todos editan `server/`) | Rename `user/authority` → `user/authority`: el paquete nombra su rol de confianza (custodia secreto + store, autentica, emite, autoriza), no un lugar — hoy corre igual en nativo que en un Worker. Elimina el hack `authority` y la colisión con el repo `tinywasm/server`. Breaking a propósito; sin consumidores externos (verificado 2026-07-14). |
